...
void SCSerial::wFlushSCS()
{
	if(txBufLen){
        ...
        if (written < 0) {
            perror("serial write");
        } else if ((size_t)written < txBufLen) {
            // Partial write: log and attempt to flush remaining bytes
            perror("serial partial write");
            // try to write remaining bytes (best-effort)
            ssize_t rem = write(fd, txBuf + written, txBufLen - written);
            if (rem < 0) perror("serial write remainder");
        }
        // reset buffer length regardless; buffer contains latest data
        txBufLen = 0;
	}
}
