...
private:
  ...
  bool commands_initialized_{false};  // Track if commands have been synced with actual positions
  int startup_write_cycles_{0};  // Count write cycles after activation for startup safety
  bool first_read_after_activate_{false};  // Track if at least one read() has occurred after activation
  int startup_read_cycles_{0};  // Count read cycles after activation for continuous command syncing
